...
```{dropdown} Can these principles apply to my subject area?
...
```{dropdown} Will applying these principles make grading take more time?
...
```{dropdown} How have students adapted to these new systems? I’m concerned that all of these different grading systems will make the students confused/anxious.
...
```{dropdown} How much pushback should I expect from students?
...
```{dropdown} My LMS doesn't support alternative grading structures.

There are some workarounds. For example, in [Victoria's web dev course](../cases/web-dev), all assignments were unweighted and worth only 1 point. The final grade calculation was points earned divided by total possible points. Similar grading can also be calculated via a spreadsheet and then current final grades can be imported to your LMS. Speaking to your IT office or LMS support team may prove beneficial for workarounds or tips and tricks!
```

```{dropdown} My LMS doesn't support peer-grading.
